...
void filter_init(struct filter* filter, int id, double fs, int nch)
{
	...
	case NOTCH50:
		filt = rtf_create_notch(nch, RTF_FLOAT, 50/fs, 12/fs);
		break;

	case NOTCH60:
		filt = rtf_create_notch(nch, RTF_FLOAT, 60/fs, 12/fs);
		break;
...
void scopetab_filter_ordercombo_cb(GtkComboBox* combo, struct filter* filter)
{
	...
	if (!combo_get_selected_value(combo, 1, &value))
		return;
...
void setup_initial_values(struct scopetab* sctab, const struct tabconf* cf)
{
	...
	sctab->filters[LOWPASS].order = 2;
	sctab->filters[HIGHPASS].order = 2;
...
void initialize_widgets(struct scopetab* sctab)
{
	...
	combo_select_int_value(GTK_COMBO_BOX(widg[LP_ORDER]), 1, sctab->filters[LOWPASS].order);
	combo_select_int_value(GTK_COMBO_BOX(widg[HP_ORDER]), 1, sctab->filters[HIGHPASS].order);
